...
void check_acc(int Nr_LP, int Nr_HP, ErrAnlyVars &errAnly,
    std::vector<int> &timeslices, double &ratio_C_LP_err, double &m_err_max,
    double &m_err_av) {
  int NumTs = timeslices.size();
  int d = num_links; // no displacement
  int g = 0; // scalar
...
void checkout(int Nr_LP, int Nr_HP, ErrAnlyVars &errAnly, std::string out_fname,
  ...
  for (int d = 0; d < num_disp; ++d) {
    QDPIO::cout << "Displacement = " << (d/num_links)*100+ ((!(d%num_links) && d/num_links)?num_links:(d%num_links)) << std::endl << std::endl;
    for (int t = 0; t < NumTs; ++t) {
      QDPIO::cout << "Timeslice = " << timeslices[t] << std::endl << std::endl;
  ...
  for (int d = 0; d < num_disp; ++d) {
    ...
    for (int t = 0; t < NumTs; ++t) {
      ...
      for (int g=0; g<NUM_G; ++g) {
        char buffer[250];
        sprintf(buffer, "%2d %3d %2d %16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n",
				(d/num_links)*100+ ((!(d%num_links) && d/num_links)?num_links:(d%num_links)),
            timeslices[t],
            g,
      ...
      for (int g = 0; g < NUM_G; ++g) {
        char buffer[250];
        sprintf(buffer, "%2d %3d %2d %16.8e %16.8e %16.8e %16.8e\n",
				(d/num_links)*100+ ((!(d%num_links) && d/num_links)?num_links:(d%num_links)) , timeslices[t], g,
          TrM_inv_av[d][g][t].elem().elem().elem().real(),
          TrM_inv_av[d][g][t].elem().elem().elem().imag(),
...
int main(int argc, char **argv) {
  ...
  try {
	...
	for (int idir = 0; idir < 2; ++idir){
		for (int link_dir=0; link_dir<4; ++link_dir){
		int mu = link_dir;
		shift_psi = psi;
		for (int d = 0; d < num_links+1; ++d) {
          if (((link_dir!=0)||(idir!=0))&&(d==0)) continue;
          chi = shift_psi;
		if (d!=0){
          if (idir == 0) {
        ...
        for (int g = 0; g < NUM_G; ++g) {
          ...
          for (int t = 0; t < NumTs; ++t) {
            ...
            errAnly.TrM_inv_sum_LP[link_dir*num_links+idir*num_links*4+d][g][t] += TrM_inv;

            // Statistical error estimation
            errAnly.TrM_inv_LP_sqsum_r[link_dir*num_links+idir*num_links*4+d][g][t] += pow(
              (double)TrM_inv.elem().elem().elem().real(), 2);
            errAnly.TrM_inv_LP_sqsum_i[link_dir*num_links+idir*num_links*4+d][g][t] += pow(
              (double)TrM_inv.elem().elem().elem().imag(), 2);

  #ifdef CALC_ERR_ERR
            TrM_inv_est_LP_sum[link_dir*num_links+idir*num_links*4+d][g][t] += TrM_inv;
  #endif
          }
        }
      }
      ...
      if (Nr_LPHP_ratio > 0 && count_lp % Nr_LPHP_ratio == 0) {
        ...
        for (int d = 0; d < num_links+1; ++d) {
          ...
          for (int g = 0; g < NUM_G; ++g) {
            ...
            for (int t = 0; t < NumTs; ++t)
              TrM_inv_C_LP[link_dir*num_links+idir*num_links*4+d][g][t] = corr_fn_t[timeslices[t]];
          }
        }
		}
		}
        ...
        for (int d = 0; d < num_links+1; ++d) {
          ...
          for (int g = 0; g < NUM_G; ++g) {
            ...
            // For the correction term, we don't need to add constant part Tr[2 kappa I]
            for (int t = 0; t < NumTs; ++t)
              TrM_inv_C_HP[link_dir*num_links+idir*num_links*4+d][g][t] = corr_fn_t[timeslices[t]];
          }
        }
		}
		}
      ...
      if (count_lp >= Min_Nr_LP) {
        ...
        for (int idx_cp = 0; idx_cp < num_checkp; ++idx_cp) {
          while (1) {
            ...
            if (chkout) {
              // checkout
              checkout(count_lp, count_hp, errAnly, checkp[idx_cp].OutFileName,
