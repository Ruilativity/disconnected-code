...
struct ErrAnlyVars 
{
  multi2d<DComplex> TrM_inv_sum_LP; 
  multi2d<DComplex> TrM_inv_sum_C;
  multi2d<double>   TrM_inv_LP_sqsum_r;
  multi2d<double>   TrM_inv_LP_sqsum_i;
  multi2d<double>   TrM_inv_C_sqsum_r;
  multi2d<double>   TrM_inv_C_sqsum_i;

#ifdef CALC_ERR_ERR
  multi2d<vector<DComplex>> TrM_inv_est;
#endif
  ...
  {
    TrM_inv_sum_LP.resize(NUM_G, NumTs); 
    TrM_inv_sum_C.resize(NUM_G, NumTs);
    TrM_inv_LP_sqsum_r.resize(NUM_G, NumTs);
    TrM_inv_LP_sqsum_i.resize(NUM_G, NumTs);
    TrM_inv_C_sqsum_r.resize(NUM_G, NumTs);
    TrM_inv_C_sqsum_i.resize(NUM_G, NumTs);

#ifdef CALC_ERR_ERR
    TrM_inv_est.resize(NUM_G, NumTs);
#endif

    for(int i=0; i<NUM_G; ++i) 
    for(int t=0; t<NumTs; ++t){
      TrM_inv_sum_LP[i][t] = zero;
      TrM_inv_sum_C[i][t]  = zero;

      TrM_inv_LP_sqsum_r[i][t] = 0.0;
      TrM_inv_LP_sqsum_i[i][t] = 0.0;
      TrM_inv_C_sqsum_r[i][t]  = 0.0;
      TrM_inv_C_sqsum_i[i][t]  = 0.0;
    }
  }
};
              ...
              double &m_err_max, double &m_err_av)
{
  ...
  if(Nr_LP != 0)
    for(int t=0; t<NumTs; ++t)
      TrM_inv_av_LP[t] = errAnly.TrM_inv_sum_LP[g][t]/(double)Nr_LP;

  if(Nr_HP != 0)
    for(int t=0; t<NumTs; ++t)
      TrM_inv_av_C[t] = errAnly.TrM_inv_sum_C[g][t]/(double)Nr_HP;
  ...
  for(int t=0; t<NumTs; ++t)
  {
    if(Nr_LP > 1) {
      ...
      TrM_inv_LP_err_r[t] = sqrt( (errAnly.TrM_inv_LP_sqsum_r[g][t]/Nr_LP - TrM_inv_av_LP_sq_r)/(Nr_LP-1.0) );
    } 
    else {
    ...
    if(Nr_HP > 1) {
      ...
      TrM_inv_C_err_r[t] = sqrt( (errAnly.TrM_inv_C_sqsum_r[g][t]/Nr_HP - TrM_inv_av_C_sq_r)/(Nr_HP-1.0) );
    }
    else {
              ...
              vector<int> &timeslices, int chkout_order, bool Restarted)
{
  ...
  // TSM estimate of Tr [ M^{-1} \gamma ]
  multi2d<DComplex> TrM_inv_av(NUM_G, NumTs);

  // Low precision estimate and correction to Tr [ G^{-1} \Gamma ]
  multi2d<DComplex> TrM_inv_av_LP(NUM_G, NumTs);
  multi2d<DComplex> TrM_inv_av_C(NUM_G, NumTs);

  // Calculate average
  if(Nr_LP != 0)
    for(int g=0; g<NUM_G; ++g)
    for(int t=0; t<NumTs; ++t)
      TrM_inv_av_LP[g][t] = errAnly.TrM_inv_sum_LP[g][t]/(double)Nr_LP;

  if(Nr_HP != 0)
    for(int g=0; g<NUM_G; ++g)
    for(int t=0; t<NumTs; ++t)
      TrM_inv_av_C[g][t] = errAnly.TrM_inv_sum_C[g][t]/(double)Nr_HP;


  //-----------------------------
  // Calculate statistical error 
  //-----------------------------
  multi2d<double> TrM_inv_LP_err_r(NUM_G, NumTs);
  multi2d<double> TrM_inv_LP_err_i(NUM_G, NumTs);
  multi2d<double> TrM_inv_C_err_r(NUM_G, NumTs);
  multi2d<double> TrM_inv_C_err_i(NUM_G, NumTs);
  multi2d<double> TrM_inv_err_r(NUM_G, NumTs);
  multi2d<double> TrM_inv_err_i(NUM_G, NumTs);

  for(int g=0; g<NUM_G; ++g)
  for(int t=0; t<NumTs; ++t)
  {
    if(Nr_LP > 1) {
      double TrM_inv_av_LP_sq_r = pow((double)TrM_inv_av_LP[g][t].elem().elem().elem().real(), 2);
      double TrM_inv_av_LP_sq_i = pow((double)TrM_inv_av_LP[g][t].elem().elem().elem().imag(), 2);
      TrM_inv_LP_err_r[g][t] = sqrt( (errAnly.TrM_inv_LP_sqsum_r[g][t]/Nr_LP - TrM_inv_av_LP_sq_r)/(Nr_LP-1.0) );
      TrM_inv_LP_err_i[g][t] = sqrt( (errAnly.TrM_inv_LP_sqsum_i[g][t]/Nr_LP - TrM_inv_av_LP_sq_i)/(Nr_LP-1.0) );
    } 
    else {
      TrM_inv_LP_err_r[g][t] = 0.0;
      TrM_inv_LP_err_i[g][t] = 0.0;
    }

    if(Nr_HP > 1) {
      double TrM_inv_av_C_sq_r = pow(TrM_inv_av_C[g][t].elem().elem().elem().real(), 2);
      double TrM_inv_av_C_sq_i = pow(TrM_inv_av_C[g][t].elem().elem().elem().imag(), 2);
      TrM_inv_C_err_r[g][t] = sqrt( (errAnly.TrM_inv_C_sqsum_r[g][t]/Nr_HP - TrM_inv_av_C_sq_r)/(Nr_HP-1.0) );
      TrM_inv_C_err_i[g][t] = sqrt( (errAnly.TrM_inv_C_sqsum_i[g][t]/Nr_HP - TrM_inv_av_C_sq_i)/(Nr_HP-1.0) );
    }
    else {
      TrM_inv_C_err_r[g][t] = 0.0;
      TrM_inv_C_err_i[g][t] = 0.0;
    }

    TrM_inv_err_r[g][t] = sqrt( pow(TrM_inv_LP_err_r[g][t], 2) + pow(TrM_inv_C_err_r[g][t], 2) );
    TrM_inv_err_i[g][t] = sqrt( pow(TrM_inv_LP_err_i[g][t], 2) + pow(TrM_inv_C_err_i[g][t], 2) );
  ...
  for(int t=0; t<NumTs; ++t){
    ...
    for(int g=0; g<NUM_G; ++g)
      QDPIO::cout << "Tr [ M^{-1} g" << g << " ]_LP = " << TrM_inv_av_LP[g][t]
        << " err( " << TrM_inv_LP_err_r[g][t] << ", " << TrM_inv_LP_err_i[g][t] << " )" << endl;

    QDPIO::cout << endl;

    for(int g=0; g<NUM_G; ++g)
      QDPIO::cout << "Tr [ M^{-1} g" << g << " ]_Cr = " << TrM_inv_av_C[g][t] 
        << " err( " << TrM_inv_C_err_r[g][t] << ", " << TrM_inv_C_err_i[g][t] << " )" << endl;
  ...
  for(int g=0; g<NUM_G; ++g)
  for(int t=0; t<NumTs; ++t)
    TrM_inv_av[g][t] = TrM_inv_av_LP[g][t] + TrM_inv_av_C[g][t];
 
  //--------------------------------------------------------------------
  // Calculate Another estimate of total error and error of error 
  //--------------------------------------------------------------------
#ifdef CALC_ERR_ERR
  multi2d<double> TrM_inv_est_av_r(NUM_G, NumTs);
  multi2d<double> TrM_inv_est_av_i(NUM_G, NumTs);
  multi2d<double> TrM_inv_est_err_r(NUM_G, NumTs);
  multi2d<double> TrM_inv_est_err_i(NUM_G, NumTs);
  multi2d<double> TrM_inv_est_err_err_r(NUM_G, NumTs);
  multi2d<double> TrM_inv_est_err_err_i(NUM_G, NumTs);
  
  for(int g=0; g<NUM_G; ++g)
  for(int t=0; t<NumTs; ++t){
    TrM_inv_est_av_r[g][t] = 0.0;
    TrM_inv_est_av_i[g][t] = 0.0;
    
    int N_e = errAnly.TrM_inv_est[g][t].size();
   
    // Calculate average
    double tmp_av_r = 0.0;
    double tmp_av_i = 0.0;
    for(int i=0; i<N_e; ++i) {
      tmp_av_r += errAnly.TrM_inv_est[g][t][i].elem().elem().elem().real();
      tmp_av_i += errAnly.TrM_inv_est[g][t][i].elem().elem().elem().imag();
    }
    tmp_av_r /= N_e;
    tmp_av_i /= N_e;

    TrM_inv_est_av_r[g][t] = tmp_av_r;
    TrM_inv_est_av_i[g][t] = tmp_av_i;
    ...
    for(int i=0; i<N_e; ++i) {
      double m_r = errAnly.TrM_inv_est[g][t][i].elem().elem().elem().real();
      double m_i = errAnly.TrM_inv_est[g][t][i].elem().elem().elem().imag();
    ...
    TrM_inv_est_err_r[g][t] = tmp_err_r;
    TrM_inv_est_err_i[g][t] = tmp_err_i;
    ...
    TrM_inv_est_err_err_r[g][t] = tmp_err_var_r / (2*tmp_err_r);
    TrM_inv_est_err_err_i[g][t] = tmp_err_var_i / (2*tmp_err_i) ;
  }// Loop over g and t
  ...
  for(int t=0; t<NumTs; ++t) {
    ...
    for(int g=0; g<NUM_G; ++g)
      QDPIO::cout << "Tr [ M^{-1} g" << g << " ]    = " << TrM_inv_av[g][t]
        << " std( " << TrM_inv_err_r[g][t] << ", " << TrM_inv_err_i[g][t] << " )"<< endl;

#ifdef CALC_ERR_ERR
    for(int g=0; g<NUM_G; ++g)
      QDPIO::cout << "Tr [ M^{-1} g" << g << " ]_est= ( " 
        << TrM_inv_est_av_r[g][t] << ", " << TrM_inv_est_av_i[g][t] << " ) "
        << " ( " << TrM_inv_est_err_r[g][t] << ", " << TrM_inv_est_err_i[g][t] << " )"
        << " ( " << TrM_inv_est_err_err_r[g][t] << ", " << TrM_inv_est_err_err_i[g][t] << " )"<< endl;
#endif
    QDPIO::cout << endl;
  } //for(int t=0; t<NumTs; ++t)
...
#ifdef CALC_ERR_ERR
    fout << "# t   g  Tr[M^-1 g_i]_re  Tr[M^-1 g_i]_im   StatErr_re      StatErr_im       StatErrErr_re      StatErrErr_im" << "\n" ;
  
    for(int g=0; g<NUM_G; ++g){
      char buffer[250];
      sprintf(buffer, "%3d %2d %16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n",
        timeslices[t],
        g,
        TrM_inv_av[g][t].elem().elem().elem().real(),
        TrM_inv_av[g][t].elem().elem().elem().imag(),
        TrM_inv_err_r[g][t],
        TrM_inv_err_i[g][t],
        TrM_inv_est_err_err_r[g][t],
        TrM_inv_est_err_err_i[g][t]);
      std::string oline(buffer);
      fout << oline;
    } // for(int g=0; g<NUM_G; ++g)
#else
    fout << "# t   g  Tr[M^-1 g_i]_re  Tr[M^-1 g_i]_im   StatErr_re      StatErr_im" << "\n" ;

    for(int g=0; g<NUM_G; ++g){
      char buffer[250];
      sprintf(buffer, "%3d %2d %16.8e %16.8e %16.8e %16.8e\n",
        timeslices[t],
        g,
        TrM_inv_av[g][t].elem().elem().elem().real(),
        TrM_inv_av[g][t].elem().elem().elem().imag(),
...
int main(int argc, char **argv)
{
  ...
  LatticeFermion chi;
#ifdef CALC_ERR_ERR
  multi2d<DComplex> TrM_inv_est_LP_sum(NUM_G, NumTs);
  for(int g=0; g<NUM_G; ++g) 
  for(int t=0; t<NumTs; ++t)
    TrM_inv_est_LP_sum[g][t] = zero;
#endif
    ...
    for(int count_lp=1; count_lp <= Max_Nr_LP; ++count_lp) {
      ...
      for(int g=0; g<NUM_G; ++g) {
        ...
        for(int t=0; t<NumTs; ++t) {
          ...
          errAnly.TrM_inv_sum_LP[g][t] += TrM_inv;

          // Statistical error estimation
          errAnly.TrM_inv_LP_sqsum_r[g][t] += pow((double)TrM_inv.elem().elem().elem().real(), 2);
          errAnly.TrM_inv_LP_sqsum_i[g][t] += pow((double)TrM_inv.elem().elem().elem().imag(), 2);

#ifdef CALC_ERR_ERR
          TrM_inv_est_LP_sum[g][t] += TrM_inv;
#endif
        }
      }
      ...
      if( Nr_LPHP_ratio > 0  &&  count_lp%Nr_LPHP_ratio == 0 ) {
        ...
        // Temp variables
        multi2d<DComplex> TrM_inv_C_HP(NUM_G, NumTs);
        multi2d<DComplex> TrM_inv_C_LP(NUM_G, NumTs);
        for(int g=0; g<NUM_G; ++g)
        for(int t=0; t<NumTs; ++t) {
          TrM_inv_C_HP[g][t] = zero;
          TrM_inv_C_LP[g][t] = zero;
        }
        ...
        for(int g=0; g<NUM_G; ++g) {
          ...
          for(int t=0; t<NumTs; ++t)
            TrM_inv_C_LP[g][t] = corr_fn_t[timeslices[t]];
        }
        ...
        for(int g =0; g<NUM_G; ++g) {
          ...
          // For the correction term, we don't need to add constant part Tr[2 kappa I]
          for(int t=0; t<NumTs; ++t)
            TrM_inv_C_HP[g][t] = corr_fn_t[timeslices[t]];
        }
        ...
        for(int g=0; g<NUM_G; ++g) 
        for(int t=0; t<NumTs; ++t){
          TrM_inv = TrM_inv_C_HP[g][t] - TrM_inv_C_LP[g][t];
          errAnly.TrM_inv_sum_C[g][t] += TrM_inv;

          // Statistical error estimation
          errAnly.TrM_inv_C_sqsum_r[g][t] += pow((double)TrM_inv.elem().elem().elem().real(), 2);
          errAnly.TrM_inv_C_sqsum_i[g][t] += pow((double)TrM_inv.elem().elem().elem().imag(), 2);

#ifdef CALC_ERR_ERR
          DComplex TrM_inv_est_tmp = TrM_inv_est_LP_sum[g][t]/Nr_LPHP_ratio + TrM_inv;
          errAnly.TrM_inv_est[g][t].push_back(TrM_inv_est_tmp);
      ...
      if(noise_input_version == 2) {
        ...
        if(count_lp == restart_NrLP && !Restarted) {
          ...
          check_acc(count_lp, count_hp, errAnly, timeslices, ratio_C_LP_err, s_err_max, s_err_av);
          ...
          if(ratio_C_LP_err > restart_factor){
            QDPIO::cout << "with new LP inverter parameters!" << endl;
      ...
      if(count_lp >= Min_Nr_LP) {
        ...
        for(int idx_cp = 0; idx_cp < num_checkp; ++idx_cp) {
          while(1) {
            ...
            if(chkout) {
              // checkout
              checkout(count_lp, count_hp, errAnly, checkp[idx_cp].OutFileName, timeslices, checkp[idx_cp].chkout_order, Restarted);
