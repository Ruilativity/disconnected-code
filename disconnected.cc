...
void checkout(int Nr_LP, int Nr_HP, ErrAnlyVars &errAnly, std::string out_fname, std::vector<int> &link_dirs, int link_max,
	...
	for (int d = 0; d < num_disp; ++d) {
		QDPIO::cout << "Displacement = " << ( d ? ((d-1)/link_max) : 0 )*100+ (d ? ((d-1)%link_max+1) : 0) << std::endl << std::endl;
		for (int t = 0; t < NumTs; ++t) {
		...
		for (int t = 0; t < NumTs; ++t) {
			...
			for (int g=0; g<NUM_G; ++g) {
				...
				sprintf(buffer, "%2d %3d %2d %16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n",
						( d ? ((d-1)/link_max) : 0 )*100+ (d ? ((d-1)%link_max+1) : 0),
						timeslices[t],
						g,
			...
			for (int g = 0; g < NUM_G; ++g) {
				...
				sprintf(buffer, "%2d %3d %2d %16.8e %16.8e %16.8e %16.8e\n",
						( d ? ((d-1)/link_max) : 0 )*100+ (d ? ((d-1)%link_max+1) : 0) , timeslices[t], g,
						TrM_inv_av[d][g][t].elem().elem().elem().real(),
						TrM_inv_av[d][g][t].elem().elem().elem().imag(),
				...
				for (int d = 0; d < link_max+1; ++d) {
					...
					for (int g = 0; g < NUM_G; ++g) {
						...
						for (int t = 0; t < NumTs; ++t) {
							...
							errAnly.TrM_inv_sum_LP[d+dir_index*link_max][g][t] += TrM_inv;
							
							// Statistical error estimation
							errAnly.TrM_inv_LP_sqsum_r[d+dir_index*link_max][g][t] += pow(
																										   (double)TrM_inv.elem().elem().elem().real(), 2);
							errAnly.TrM_inv_LP_sqsum_i[d+dir_index*link_max][g][t] += pow(
