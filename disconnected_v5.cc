...
struct ErrAnlyVars {
	...
	ErrAnlyVars(int NumDisp, int NumMom, int NumTs) {
		TrM_inv_sum_LP.resize(NumDisp, NumMom, NUM_G, NumTs);
		TrM_inv_sum_C.resize(NumDisp, NumMom, NUM_G, NumTs);
		TrM_inv_LP_sqsum_r.resize(NumDisp, NumMom, NUM_G, NumTs);
		TrM_inv_LP_sqsum_i.resize(NumDisp, NumMom, NUM_G, NumTs);
		TrM_inv_C_sqsum_r.resize(NumDisp, NumMom, NUM_G, NumTs);
		TrM_inv_C_sqsum_i.resize(NumDisp, NumMom, NUM_G, NumTs);
		
#ifdef CALC_ERR_ERR
		TrM_inv_est.resize(NumDisp, NumMom, NUM_G, NumTs);
#endif
...
void check_acc(int Nr_LP, int Nr_HP, ErrAnlyVars &errAnly, std::vector<int> &link_dirs,
			   ...
			   double &m_err_max, double &m_err_av, SftMom phases) {
...
void checkout(int Nr_LP, int Nr_HP, ErrAnlyVars &errAnly, std::string out_fname, std::string lamet_out_fname, std::vector<int> &link_dirs, int link_max,
			  std::vector<int> &timeslices, int chkout_order, bool Restarted, SftMom phases) {
	int NumTs = timeslices.size();
