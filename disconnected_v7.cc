...
void read(XMLReader& xml, const std::string& path, NoiseSource_t& p) {
    ...
    if (paramtop.count("SpinDilution") > 0) {
        read(paramtop, "SpinDilution", p.spin_dilute);
    } else {
        p.spin_dilute = false;
    }
    if (paramtop.count("ColorDilution") > 0) {
        read(paramtop, "ColorDilution", p.color_dilute);
    } else {
        p.color_dilute = false;
    }
...
int main(int argc, char **argv) {
    ...
    bool spin_dilute = input.param.noise_src.spin_dilute;
    bool color_dilute = input.param.noise_src.color_dilute;
    int max_spin_comp=1;
    int max_color_comp=1;
    ...
    if (spin_dilute) {
        QDPIO::cout << "Generate diluting spin sources"<< std::endl;
        max_spin_comp=Ns;
    }
    
    if (color_dilute) {
        QDPIO::cout << "Generate diluting color sources"<< std::endl;
        max_color_comp=Nc;
    }
