...
// This version of code works for the original chroma where the SftMom function takes mom2_max as input.
...
struct Params_t {
	...
	int mom2_max_local;
	int mom2_max_1st_mom;
	int mom2_max_2nd_mom;
	int mom2_max_lamet;
...
void read(XMLReader& xml, const std::string& path, Params_t& p) {
	...
	if (paramtop.count("mom2_max_local") > 0)
		read(paramtop, "mom2_max_local", p.mom2_max_local);
	else {
		p.mom2_max_local=0;
	}
	if (paramtop.count("mom2_max_1st_mom") > 0)
		read(paramtop, "mom2_max_1st_mom", p.mom2_max_1st_mom);
	else {
		p.mom2_max_1st_mom=0;
	}
	if (paramtop.count("mom2_max_2nd_mom") > 0)
		read(paramtop, "mom2_max_2nd_mom", p.mom2_max_2nd_mom);
	else {
		p.mom2_max_2nd_mom=0;
	}
	if (paramtop.count("mom2_max_lamet") > 0)
		read(paramtop, "mom2_max_lamet", p.mom2_max_lamet);
	else {
		p.mom2_max_lamet=0;
	}
